...
class ImageDataset(Dataset):
    ...
    def init(self, root_dir, trainPercent, valPercent, flag, transform = None):
...
#        np.random.shuffle(trainID)
        lengths = [int(len(self.trainID)*trainPercent), int(len(self.trainID)*(trainPercent+valPercent))]#, (len(self.trainID) - (int(len(self.trainID)*trainPercent)+ int(len(self.trainID)*valPercent)))]
#        train = trainID[:lengths[0]]
#        val   = trainID[lengths[0]:lengths[1]]
#        test  = trainID[lengths[1]:]
        if flag == 'train':
            self.imgID = trainID[:lengths[0]]
            self.trainCap = trainCaps[:lengths[0]]
        elif flag == 'validation':
            self.imgID = trainID[lengths[0]:lengths[1]]
            self.trainCap = trainCaps[lengths[0]:lengths[1]]
        elif flag == 'test':
